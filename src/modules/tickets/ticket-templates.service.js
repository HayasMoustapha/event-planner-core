...
class TicketTemplatesService {
  ...
  async updateTemplate(templateId, updateData, userId) {
    try {
      ...
      const updateResult = await ticketTemplatesRepository.update(templateId, updateData, userId);
      
      if (!updateResult.success) {
        return {
          success: false,
          error: updateResult.error || 'Failed to update template'
        };
      }
  ...
  async validateTemplateForEvent(templateId, eventId) {
    try {
      ...
      // Validation logique pour l'événement
      const isSuitable = template.is_active; // Template doit être actif
      ...
      return {
        ...
        data: {
          ...
          recommendations: isSuitable 
            ? ['Template compatible avec cet événement', 'Aucune action requise']
            : ['Template non actif', 'Veuillez activer le template avant utilisation']
        }
      };
    } catch (error) {
  ...
  async cloneTemplate(templateId, newName, userId) {
    try {
      ...
      const clonedTemplateData = {
        ...
        description: originalTemplate.description ? `Copie de: ${originalTemplate.description}` : null,
      ...
      const clonedTemplate = await ticketTemplatesRepository.create(clonedTemplateData);
