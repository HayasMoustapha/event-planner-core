...
class AdminRepository {
  ...
  async getUserActivity(userId) {
    const query = `
      SELECT 
        COUNT(CASE WHEN e.id IS NOT NULL THEN 1 END) as event_count,
        COUNT(CASE WHEN e.status = 'published' THEN 1 END) as published_events,
        COUNT(CASE WHEN eg.id IS NOT NULL THEN 1 END) as guest_count,
        COUNT(CASE WHEN t.id IS NOT NULL THEN 1 END) as ticket_count
      FROM users u
      LEFT JOIN events e ON u.id = e.organizer_id
      LEFT JOIN event_guests eg ON u.id = eg.guest_id
    ...
    return {
      success: true,
      data: {
        hasActiveEvents: activity.event_count > 0,
        publishedEvents: activity.published_events,
        guestCount: activity.guest_count,
        ticketCount: activity.ticket_count
      }
    };
  }
  ...
  async createBackup(backupData) {
    const query = `
      INSERT INTO system_backups (
        id, type, status, include_data, created_by, created_at, 
        estimated_size, details
      )
    ...
    const values = [
      ...
      backupData.type,
      backupData.status,
      backupData.include_data,
      backupData.created_by,
      backupData.created_at,
      backupData.estimated_size,
      JSON.stringify(backupData.details || {})
    ];
