/**
 * SCRIPT DE TEST D'INTÉGRATION PAYMENT SERVICE - VERSION CORRIGÉE
 * 
 * OBJECTIF : Tester la communication entre event-planner-core et payment-service avec authentification correcte
 * Ce script utilise le bon format d'authentification pour les services
 * 
 * UTILISATION :
 * node test-payment-integration-fixed.js
 * 
 * PRÉREQUIS :
...
// Token d'authentification de service (format correct)
...
/**
 * Fonction pour tester une route API avec authentification de service
 */
async function testRoute(method, url, data = null, description = '', withAuth = true) {
  try {
    ...
    // Ajouter l'en-tête d'authentification de service si nécessaire
    if (withAuth) {
...
async function runAllTests() {
  log('🚀 DÉMARRAGE DES TESTS D\'INTÉGRATION PAYMENT - VERSION CORRIGÉE', 'yellow');
  log('==============================================================', 'yellow');
